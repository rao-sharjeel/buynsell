...
export function* REGISTER({ payload }) {
  const { email, password, confirmPassword, username, language, zipcode } = payload
  yield put({
    type: 'user/SET_STATE',
    payload: {
      loading: true,
  ...
  const success = yield call(mapAuthProviders[authProvider].register, username, email, password, zipcode, language)
  console.log("🚀 ~ file: sagas.js ~ line 76 ~ function*REGISTER ~ success", success)
  if (success) {
    // yield put({
    //   type: 'user/LOAD_CURRENT_ACCOUNT',
    // })
